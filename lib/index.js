...
async function processImagesInHtml(htmlContent, imageArrays, imageType) {
...
async function processScssFiles(scssContent, imageArrays, imageType) {
...
async function processHtmlFiles(directory, imageType) {
    const imageArrays = {
        png: [],
        jpg: [],
        webp: [],
            ...
            for (const file of files) {
                ...
                if (stats.isDirectory()) {
                    ...
                    if (file.endsWith('.html')) {
                        ...
                        await processImagesInHtml(htmlContent, imageArrays, imageType);
                    } else if (file.endsWith('.scss')) {
                        const scssContent = await fs.readFile(filePath, 'utf-8');
                        await processScssFiles(scssContent, imageArrays, imageType);
                    }
                }
            }
        } catch (error) {
...
async function processImagesInDirectory(imageDirectory, imageType) {
    const imageArrays = {};
    ...
    try {
        ...
        for (const file of imageFiles) {
            ...
            if ((await fs.stat(filePath)).isDirectory()) {
                const subdirectoryArrays = await processImagesInDirectory(filePath, imageType);
...
async function main() {
    try {
        ...
        const imageType = process.argv[2] || 'svg'; // Get image type from command line argument, default to 'svg' if not provided
        const htmlImages = await processHtmlFiles(path.join(rootDirectory, '/'), imageType);

        const assetsDirectory = path.join(process.cwd(), 'assets');
        const resultImages = await processImagesInDirectory(path.join(assetsDirectory, 'images'), imageType);
        const resultSvgIcons = await processImagesInDirectory(path.join(assetsDirectory, 'svg-icons'), imageType);
